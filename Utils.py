...
def download_Mkt_Data_For_Symbols_Without_Start_And_EndDate_With_Granularity_And_SaveFilesToDirectory(symbolList, granularity, dirname, format):
    start_date = calculate_dates(get_current_date(), 1000, 1000)['past_date']
...
def filter_dataframe_by_days(dataframe, filterTypeDays):
    dataframe['High'] = dataframe['High'].rolling(window=filterTypeDays).max()
    dataframe['Low'] = dataframe['Low'].rolling(window=filterTypeDays).min()
    dataframe['Open'] = dataframe['Open'].shift(filterTypeDays)
    dataframe['Volume'] = dataframe['Volume'].rolling(window=filterTypeDays).sum()
    filtered_dataframe = dataframe.iloc[::filterTypeDays].copy()
